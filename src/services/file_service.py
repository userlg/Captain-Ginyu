...
def get_folders(root_path: Path) -> list[str]:
    ...
    try:
        ...
        folders = [
            item
            for item in content
            if os.path.isdir(root_path / item) and not item.startswith(".") and item not in excluded
        ]
...
def process_files(folders: list[str], root_path: Path) -> ProcessingResult:
    ...
    index = 0 if config.malocclusion_keyword in folders[0] else identify_starting_index(folders)
    ...
    for folder in folders:
        try:
            ...
            for file in files:
                ...
                if (
                    file.endswith(config.file_extension)
